...
const toBook = (row: Record<string, unknown>) => ({
  ...row,
...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (request.method === 'GET') {
    ...
    return Response.json(result.results.map((row) => toBook(row as Record<string, unknown>)));
  }
